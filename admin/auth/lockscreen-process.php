...
if ($attempts >= 10) {
    logAudit(
        ...
        'auth.lockscreen_force_logout', 'employees',
...
if (!validateAdminCsrf($csrf)) {
    adminRedirect('/admin/lockscreen', ['msg' => 'error']);
}
...
if (!preg_match('/^\d{6}$/', $code)) {
    ...
    adminRedirect('/admin/lockscreen', ['msg' => 'wrong']);
}
...
if (!$valid) {
    ...
    logAudit($admin_id, null, 'auth.lockscreen_wrong_code',
        'employees', $admin_id,
        ['attempts' => $attempts + 1], null);
...
// Log de auditoria
logAudit($admin_id, null, 'auth.lockscreen_unlocked',
    'employees', $admin_id, null, null);
