...
if (!defined('ADMIN_LOGIN_URL'))     define('ADMIN_LOGIN_URL',     APP_URL . '/admin/login');
if (!defined('ADMIN_PANEL_URL'))     define('ADMIN_PANEL_URL',     APP_URL . '/admin');
...
function startAdminSession(): void
{
    if (session_status() === PHP_SESSION_NONE) {
        ...
        session_set_cookie_params([
            ...
            'path'     => '/wasomupfy/admin',  // Cookie restrito à pasta admin
...
function requireAdminLogin(): void
{
    if (!isAdminLoggedIn()) {
        ...
        adminRedirect('/admin/login', $return ? ['return' => $return] : []);
    }
}
...
function checkAdminRememberMe(): void
{
    ...
    if (!$admin) {
        ...
        setcookie(ADMIN_COOKIE_NAME, '', ['expires' => 1, 'path' => '/wasomupfy/admin']);
        return;
    }
    ...
    logAudit(
        (int)$admin['id_employees'], null,
        'auth.auto_login', 'employees', (int)$admin['id_employees'],
        null, null
    );
}
...
{
    $link    = APP_URL . '/admin/reset-password?token=' . urlencode($token);
    ...
    if (ini_get('session.use_cookies')) {
        ...
        setcookie(
            ...
            ['expires' => time() - 42000, 'path' => $p['path'],
             'domain'  => $p['domain'],   'secure' => $p['secure'],
