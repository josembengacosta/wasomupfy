...
// Arquivo: wu-panel/pages/finances/plans-process.php
// ══════════════════════════════════════════════════════════════
...
function getPlanUsageSummary(PDO $db, int $planId): array
{
    $usageMap = [
        'utilizadores com este plano selecionado' => "SELECT COUNT(*) FROM _users WHERE plan_selected = ?",
        'subscrições ligadas a este plano' => "SELECT COUNT(*) FROM _user_plan WHERE id_plan = ?",
        'pagamentos ligados a este plano' => "SELECT COUNT(*) FROM _payment WHERE id_plan = ?",
        'lançamentos ligados a este plano' => "SELECT COUNT(*) FROM _album WHERE id_plan = ?",
    ];

    $usage = [];

    foreach ($usageMap as $label => $sql) {
        $stmt = $db->prepare($sql);
        $stmt->execute([$planId]);
...
if ($action === 'delete_plan') {
    ...
    $planStmt = $db->prepare("SELECT id_plan FROM _plans WHERE id_plan = ? LIMIT 1");
    $planStmt->execute([$id]);
    if (!$planStmt->fetchColumn()) {
        jsonOut(false, 'Plano não encontrado.');
    }

    $usage = getPlanUsageSummary($db, $id);
    if ($usage) {
        jsonOut(false, 'Não é possível eliminar este plano porque existem registos vinculados: ' . implode(', ', $usage) . '.');
    }
    ...
    try {
        ...
        error_log('[PLAN DELETE] ' . $e->getMessage());

        if (($e->errorInfo[1] ?? null) == 1451) {
            jsonOut(false, 'Não é possível eliminar este plano porque ele já está vinculado a outros registos.');
        }

        jsonOut(false, 'Erro ao eliminar o plano. Tente novamente.');
    }
}
