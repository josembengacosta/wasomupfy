...
$recovery_key = bin2hex(random_bytes(11)); // 22 caracteres

// ── Gerar token de convite ──
$invite_token         = null;
$invite_token_expires = null;
if ($send_invite) {
    $invite_token         = bin2hex(random_bytes(32)); // 64 caracteres
...
try {
    ...
    // 1. Inserir em _employees (AGORA COM country_employees e city_employees)
    $ins = $db->prepare("
        INSERT INTO _employees
            (first_name, second_name, user_employees, gender, tel_employees,
             ...
             photo_employees, country_employees, city_employees)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $ins->execute([
        $first_name,
    ...
    // 2. Inserir em _employees_security (AGORA COM invited_by)
    $ins_sec = $db->prepare("
        INSERT INTO _employees_security
            (id_employees, recovery_key, invite_token, invite_token_expires, invite_used, invited_by)
        VALUES (?, ?, ?, ?, 0, ?)
    ");
...
// ── Log de auditoria ──
logAudit($admin_id, null, 'employees.created', 'employees', $new_id, null, [
    'role'   => $role,
    'status' => $status,
    'email'  => $email,
]);
...
if ($send_invite && $invite_token) {
    ...
    $status_label = $status === 'active'
        ? 'Activa — podes aceder imediatamente'
        : 'Em processo — aguarda activação pelo administrador';

    $admin_creator_name = $admin_fullname ?: $admin_name;
        ...
        <tr>
          <td style="background:#fff;padding:28px 28px 24px">
            ...
            <p style="font-size:.84rem;color:#555;line-height:1.6;margin:0 0 20px">
              <strong>' . htmlspecialchars($admin_creator_name) . '</strong> convidou-te para fazer parte da equipa <strong>Wasom Upfy</strong> como <strong>' . htmlspecialchars($role_label) . '</strong>.
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px">
              <tr>
                <td align="center">
                  <a href="' . htmlspecialchars($invite_url) . '" style="display:inline-block;background:linear-gradient(135deg,#FF0089,#cc006e);color:#fff;font-weight:700;font-size:.9rem;padding:14px 36px;border-radius:50px;text-decoration:none;letter-spacing:.3px">
            ...
            <div style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;padding:12px 14px;margin-bottom:16px">
              <p style="margin:0;font-size:.78rem;color:#7c4800;line-height:1.5">
                <strong>⚠ Segurança:</strong> Não partilhes esta senha com ninguém. A senha acima é temporária e deverás alterá-la após o primeiro acesso. Se não reconheces este convite, ignora este e-mail.
              </p>
            </div>
          </td>
        </tr>
        <tr>
          <td style="background:#f8f7fc;border-radius:0 0 16px 16px;padding:16px 24px;text-align:center;border-top:1px solid #e8e8f0">
