// ══════════════════════════════════════════════════════
// WASOM UPFY v2.0 — PWA Dashboard Manager
// Arquivo: dashboard/js/pwa-dashboard.js
// ══════════════════════════════════════════════════════

// ── Protecção contra dupla execução ──
if (typeof WasomInstallManager === 'undefined' && typeof WasomNativeManager === 'undefined') {
                            ...
                            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                                if (window.WasomPWA) {
                                    window.WasomPWA.showToast(
                                        '<i class="bi bi-arrow-repeat me-1"></i> Nova versão disponível — <a href="javascript:location.reload()" class="text-white fw-bold">Actualizar</a>',
                                        'info',
                                        0
                                    );
                                }
    ...
    class WasomNativeManager {
        ...
        _showSplash() {
            ...
            if (document.readyState === 'complete') {
                ...
                window.addEventListener('load', () => setTimeout(removeSplash, 1200), { once: true });
            }
        }
        ...
        async _syncBadge() {
            try {
                const res = await fetch('/dashboard/ajax/notifications_api?action=unread_count', {
                    credentials: 'include'
                });
                if (!res.ok) return;
        ...
        showToast(message, type = 'info', duration = 3000) {
            ...
            if (duration === 0) {
                ...
                close.style.cssText = 'background:none;border:none;color:inherit;cursor:pointer;margin-left:8px;padding:0;font-size:.85rem;';
                close.onclick = () => {
    ...
    window.WasomPWA = {
        showToast: function (msg, type, duration) {
            if (window._wasomNative) window._wasomNative.showToast(msg, type, duration);
        }
    };
