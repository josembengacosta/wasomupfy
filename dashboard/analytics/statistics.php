<head>
    ...
    <title>Estatísticas — Wasom Upfy</title>
...
<body>
    ...
    <div class="container my-4">
            ...
            <div class="row align-items-center mb-4">
                <div class="col-md-8">
                    <div class="page-header-compact">
                        ...
                        <p class="lead">
                            O Stream Analytics agrega seus streams totais no Spotify, Apple Music, Deezer,
                            Amazon e YouTube Art Tracks, calculando streams diários e mensais.
                        </p>
                    </div>
                </div>
                <div class="col-md-4 text-md-end mt-3 mt-md-0">
                    <button class="btn btn-secondary" onclick="window.location='export'">
                        <i class="bi bi-download"></i> Exportar
                    </button>
                    <button class="btn btn-pink" onclick="window.location='compare'">
                        <i class="bi bi-calendar-range"></i> Comparar períodos
                    </button>
                </div>
            </div>
            <!-- Ícone decorativo: dupla leitura -->
            <style>
                .page-header::before {
                    content: '\F26F';
                    /* bi-bar-chart-fill */
        ...
        <div class="date-range row">
            <div class="col-md-3">
                ...
                <div class="input-group">
                    ...
                    <input type="date" id="startDate" value="2024-10-09" onchange="validateDates()" />
                </div>
            </div>
            ...
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <i class="bi bi-calendar"></i>
                    <input type="date" id="endDate" value="2024-12-30" onchange="validateDates()" />
                </div>
            </div>

            <div class="col-md-3">
                <label for="dateRange">Parceiros</label>
                <div class="input-group">
            ...
            <div class="col-md-3 mt-4">
                <button class="btn-apply" id="applyButton" onclick="updateChart()" disabled>
                    Aplicar
                </button>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <ul>
                    <li>
                        ...
                        <span class="stream-count">15,000 Streams</span>
                    </li>
                    <li>
                        <div class="platform-info">
                        ...
                        <span class="stream-count">3,000 Streams</span>
                    </li>
                    <li>
                        <div class="platform-info">
                        ...
                        <span class="stream-count">9,000 Streams</span>
                    </li>
                    <li>
                        <div class="platform-info">
                        ...
                        <span class="stream-count">829 Streams</span>
                    </li>
                    <li>
                        <div class="platform-info">
                        ...
                        <span class="stream-count">80,000 Streams</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <!-- Playlists Table Card -->
        <div class="table-card mb-4">
            <div class="card" id="playlist">
                <div class="card-header">
                    <h6>Total de playlists: 3</h6>
                </div>
                <div class="filter-group">
                    <div class="input-group">
                        <i class="bi bi-search"></i>
                        <input type="text" id="playlistSearch" placeholder="Pesquisar playlist"
                            onkeyup="filterPlaylists()" />
                    </div>
                    <div class="input-group">
                        <i class="bi bi-filter"></i>
                        <input type="number" id="playlistStreamFilter" placeholder="Streams mínimos" min="0"
                            oninput="validatePlaylistStreamFilter()" />
                    </div>
                    <button class="btn-apply" id="applyPlaylistFilter" onclick="filterPlaylists()" disabled>
                        Aplicar
                    </button>
                </div>
                <div class="table-responsive">
        ...
        <div class="territory-card mb-4">
            <div class="card" id="country">
                <div class="card-header">
                    <h6>Total de territórios: 4</h6>
                </div>
                <div id="map"></div>
                <div class="filter-group">
                    <div class="input-group">
                        <i class="bi bi-search"></i>
                        <input type="text" id="territorySearch" placeholder="Pesquisar território"
                            onkeyup="filterTerritories()" />
                    </div>
                    <div class="input-group">
                        <i class="bi bi-filter"></i>
                        <input type="number" id="territoryStreamFilter" placeholder="Streams mínimos" min="0"
                            oninput="validateTerritoryStreamFilter()" />
                    </div>
                    <div class="input-group">
                        <i class="bi bi-sort-down"></i>
                        <select id="territorySort" onchange="filterTerritories()">
                            <option value="desc">Ordenar por streams (Decrescente)</option>
                            <option value="asc">Ordenar por streams (Crescente)</option>
                        </select>
                    </div>
                    <button class="btn-apply" id="applyTerritoryFilter" onclick="filterTerritories()" disabled>
                        Aplicar
                    </button>
                </div>
                <ul id="territoryList"></ul>
            </div>
        </div>
    </div>
    ...
    <script>
        // Dados completos para o gráfico (simulados)
        const fullData = {
            labels: [
                "2024-10-09",
                "2024-10-16",
            ...
            datasets: [{
                    label: "Spotify",
                    data: [
                        7000, 7200, 6800, 6500, 6000, 5800, 5500, 5000, 4800, 4500, 4200,
                        4000, 3000,
                    ],
                    borderColor: "#1db954",
                ...
                },
                {
                    label: "Deezer",
                    data: [
                        5000, 4800, 4600, 4400, 4200, 4000, 3800, 3500, 3200, 3000, 2800,
                        2500, 1000,
                    ],
                    borderColor: "#ff0089",
                ...
                },
                {
                    label: "Apple Music",
                    data: [
                        3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800, 4000, 4500,
                        4800, 5000,
                    ],
                    borderColor: "#ff4d4d",
        ...
        // Inicializar DataTables para Artistas
        let artistsTable;
        $(document).ready(function() {
            artistsTable = $("#artistsTable").DataTable({
                paging: true,
                searching: true,
                ordering: true,
                info: true,
                ...
                pageLength: 3,
                language: {
                    search: "Pesquisar artista:",
                    info: "Mostrando _START_ a _END_ de _TOTAL_ artistas",
                    paginate: {
                        next: "Próximo",
                        previous: "Anterior",
                    },
                ...
                columnDefs: [{
                        targets: 0, // Coluna de capa
                        orderable: false,
                        render: function(data, type, row) {
                            return `<img src="${row[0]}" alt="${row[1]}" style="width: 40px; height: 40px; border-radius: 5px;">`;
                        },
                    },
                    {
                        targets: 3, // Coluna de detalhes
                        orderable: false,
                        render: function(data, type, row) {
                            return `<a href="artist-details?artist=${encodeURIComponent(
                row[1]
              )}" class="details-link" onclick="console.log('Redirecionando para artista: ${row[1]
            ...
            artistsData.forEach((artist) => {
                artistsTable.row.add([
                    ...
                    "", // Coluna de detalhes será renderizada pelo columnDefs
                ]);
            });
            artistsTable.draw();
        });
        ...
        // Validar datas e habilitar/desabilitar botão Aplicar
        ...
        function updateChart() {
            ...
            if (partner !== "all") {
                filteredData.datasets = filteredData.datasets.filter((dataset) =>
                    dataset.label.toLowerCase().includes(partner)
                );
            }
        ...
        // Inicializar DataTables para Playlists
        let playlistsTable;
        $(document).ready(function() {
            playlistsTable = $("#playlistsTable").DataTable({
                ...
                pageLength: 3,
                language: {
                    info: "Mostrando _START_ a _END_ de _TOTAL_ playlists",
                    paginate: {
                        next: "Próximo",
                        previous: "Anterior",
                    },
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function(data, type, row) {
                            return `<a href="playlist-details?playlist=${encodeURIComponent(
                row[1]
              )}" class="details-link" onclick="console.log('Redirecionando para playlist: ${row[1]
                }')"><i class="bi bi-chevron-right"></i></a>`;
                        },
                    },
                ],
        ...
        function validatePlaylistStreamFilter() {
            const streamFilter = document.getElementById(
                "playlistStreamFilter"
            ).value;
            const applyButton = document.getElementById("applyPlaylistFilter");
            applyButton.disabled =
                streamFilter === "" || isNaN(streamFilter) || streamFilter < 0;
        }
        ...
        function filterPlaylists() {
            const searchTerm = document
                ...
                .value.toLowerCase();
            const streamFilter =
                parseInt(document.getElementById("playlistStreamFilter").value) || 0;
        ...
        function addMarkers(filteredTerritories) {
            ...
            filteredTerritories.forEach((territory) => {
                ...
                marker.bindPopup(
                    `<b>${territory.name}</b><br>${territory.streams} Streams`
                );
                markers.push({
                    name: territory.name,
                    marker,
            ...
            if (filteredTerritories.length > 0) {
                const bounds = L.latLngBounds(
                    filteredTerritories.map((t) => t.coords)
                );
        ...
        function validateTerritoryStreamFilter() {
            const streamFilter = document.getElementById(
                "territoryStreamFilter"
            ).value;
            const applyButton = document.getElementById("applyTerritoryFilter");
            applyButton.disabled =
                streamFilter === "" || isNaN(streamFilter) || streamFilter < 0;
        }
        ...
        function filterTerritories() {
            const searchTerm = document
                .getElementById("territorySearch")
                .value.toLowerCase();
            const streamFilter =
                parseInt(document.getElementById("territoryStreamFilter").value) || 0;
            const sortOrder = document.getElementById("territorySort").value;
            const territoryList = document.getElementById("territoryList");

            let filteredTerritories = territories.filter(
                (territory) =>
                territory.name.toLowerCase().includes(searchTerm) &&
                territory.streams >= streamFilter
            );
            ...
            filteredTerritories.forEach((territory) => {
                ...
                li.innerHTML = `
                    <div class="territory-info">
                        <img src="${territory.flag}" alt="${territory.name}">
                        <span>${territory.name}</span>
                    </div>
                    <span class="stream-count">${territory.streams
          } Streams</span>
                    <a href="country-details?country=${encodeURIComponent(
            territory.name
          )}" class="details-link" onclick="console.log('Redirecionando para ${territory.name
          }')"><i class="bi bi-chevron-right"></i></a>
                `;
                territoryList.appendChild(li);
            });

            addMarkers(filteredTerritories);
        }

        // Inicializar a lista de territórios
        filterTerritories();
    </script>
</body>
