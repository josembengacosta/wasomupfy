...
document.getElementById("genre").addEventListener("change", function () {
  ...
  if (subgenres[genre]) {
    subgenres[genre].forEach((subgenre) => {
      ...
      option.textContent =
        subgenre.charAt(0).toUpperCase() + subgenre.slice(1).replace(/_/g, " ");
...
function getFormState() {
  return {
    ...
    copyright: `© ${document.getElementById("copyright-year").value} ${
      document.getElementById("copyright-holder").value
    }`.trim(),
    phonogram: `℗ ${document.getElementById("phonogram-year").value} ${
      document.getElementById("phonogram-holder").value
    }`.trim(),
    track_count: document.getElementById("track_count").value,
    cover: document.getElementById("cover").files[0]
      ? document.getElementById("cover").files[0].name
      : "",
  };
}
  ...
  .addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        draftId = null;
        lastSavedState = {};
        window.location.href = `draft-release?id=${result.id}`;
      } else {
