...
function preencherDraftFromDB(draft) {
  ...
  if (album.title_album) {
    ...
    if (titleMatch[2]) {
      ...
      const option = Array.from(versionSelect.options).find(
        (opt) => opt.value === titleMatch[2]
      );
  ...
  if (album.type_album)
    document.getElementById("type_album").value = album.type_album;
  if (album.language)
    document.getElementById("language").value = album.language;
  ...
  if (album.label_name)
    document.getElementById("label_name").value = album.label_name;
  if (album.copyright_c)
    document.getElementById("copyright-year").value =
      album.copyright_c.match(/\d{4}/)?.[0] || "";
  if (album.copyright_p)
    document.getElementById("phonogram-year").value =
      album.copyright_p.match(/\d{4}/)?.[0] || "";
    ...
    function adicionarFaixaDBComDelay(index) {
      ...
      setTimeout(() => {
        ...
        if (track.title_track)
          card.querySelector(".track-title").value = track.title_track;
        if (track.language)
          card.querySelector(".track-language").value = track.language;
        if (track.recording_date)
          card.querySelector(".track-recording-date").value =
            track.recording_date;
        if (track.explicit)
          card.querySelector(".track-explicit").value = track.explicit;
  ...
  if (album.release_date)
    document.getElementById("release-date").value = album.release_date;
...
function preencherRascunho(draft) {
  ...
  if (draft.type_album)
    document.getElementById("type_album").value = draft.type_album;
  if (draft.language)
    document.getElementById("language").value = draft.language;
  ...
  if (draft.label_name)
    document.getElementById("label_name").value = draft.label_name;
  if (draft.copyright_year)
    document.getElementById("copyright-year").value = draft.copyright_year;
  if (draft.phonogram_year)
    document.getElementById("phonogram-year").value = draft.phonogram_year;
  ...
  if (draft.release_date)
    document.getElementById("release-date").value = draft.release_date;
  ...
  if (step === 3) {
    ...
    cards.forEach((card, i) => {
      ...
      if (!mainArtists || mainArtists.length === 0) {
        toastr.error(
          `Faixa ${i + 1}: seleciona pelo menos um artista principal.`
        );
        ok = false;
      }
      ...
      if (!audioFile) {
        ...
        toastr.error(`Faixa ${i + 1}: formato inválido. Usa MP3.`);
        ok = false;
      } else if (audioFile.size > 20 * 1024 * 1024) {
        toastr.error(`Faixa ${i + 1}: arquivo muito grande (máx. 20 MB).`);
        ok = false;
      }
    });
    return ok;
...
document.getElementById("cover-input").addEventListener("change", function (e) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    errEl.textContent = "Imagem demasiado grande (máx. 5 MB).";
    errEl.classList.remove("d-none");
    return;
  }
  ...
  reader.onload = (ev) => {
    ...
    img.onload = () => {
      if (img.width < 3000 || img.height < 3000) {
        errEl.textContent = `Imagem muito pequena (${img.width}×${img.height}px). Mínimo 3000×3000 px.`;
        errEl.classList.remove("d-none");
        return;
      }
...
document.getElementById("btn-crop-confirm").addEventListener("click", () => {
  if (!cropper) return;
  cropper.getCroppedCanvas({ width: 3000, height: 3000 }).toBlob(
    (blob) => {
...
document
  .getElementById("artist-photo-input")
...
async function saveArtist() {
  ...
  if (!name) {
    fb.innerHTML =
      '<div class="alert alert-danger py-2 small">O nome artístico é obrigatório.</div>';
  ...
  fd.append(
    "real_name",
    document.getElementById("artist-real-name").value.trim()
  );
  ...
  fd.append(
    "genre_secondary",
    document.getElementById("artist-genre-secondary").value.trim()
  );
  ...
  fd.append(
    "spotify_url",
    document.getElementById("artist-spotify").value.trim()
  );
  fd.append(
    "website_url",
    document.getElementById("artist-apple").value.trim()
  );
  fd.append(
    "youtube_url",
    document.getElementById("artist-youtube").value.trim()
  );
  const photo = document.getElementById("artist-photo-input").files[0];
  if (photo) fd.append("photo", photo);

  try {
    const res = await fetch(BASE_URL + "/creat_release_process", {
      method: "POST",
      body: fd,
    });
    ...
    if (data.ok) {
      ...
      fb.innerHTML = `<div class="alert alert-danger py-2 small">${
        data.message || "Erro ao criar artista."
      }</div>`;
      fb.classList.remove("d-none");
    }
  } catch {
    fb.innerHTML =
      '<div class="alert alert-danger py-2 small">Erro de ligação. Tenta novamente.</div>';
    fb.classList.remove("d-none");
  } finally {
...
function addTrack() {
  ...
  if (trackCount > 1)
    card.querySelector(".btn-remove-track").classList.remove("d-none");
...
function renumberTracks() {
  document.querySelectorAll(".track-card").forEach((card, i) => {
    ...
    card
      .querySelector(".btn-remove-track")
      .classList.toggle("d-none", trackCount <= 1);
  });
}
...
function updateTrackUI() {
  document.getElementById(
    "track-counter"
  ).textContent = `${trackCount} / ${UI_MAX_TRACKS}`;
...
function toggleStore(card) {
  ...
  card.querySelector(".store-checkbox").checked =
...
function buildReview() {
  ...
  document.getElementById("rev-type").textContent = `${type_alb}${
    version ? " — " + version : ""
  }`;
  document.getElementById("rev-artists").textContent = artistNames || "—";
  document.getElementById("rev-genre").textContent =
    genreText + (subText ? " › " + subText : "");
  document.getElementById("rev-tracks").textContent = `${tracks.length} faixa${
    tracks.length !== 1 ? "s" : ""
  }`;
  ...
  document.getElementById(
    "rev-stores"
  ).textContent = `${storeCount} plataforma${storeCount !== 1 ? "s" : ""}`;
...
async function submitRelease() {
  ...
  for (let i = 0; i < trackCards.length; i++) {
    ...
    if (!audioFile.type.includes("mpeg") && !fileName.endsWith(".mp3")) {
      toastr.error(`Faixa ${i + 1}: formato inválido. Usa MP3.`);
      btn.innerHTML = '<i class="bi bi-send-fill me-2"></i>Distribuir';
      btn.disabled = false;
      return;
  ...
  const maxTotalSize = 24 * 1024 * 1024; // 24 MB para áudios (30MB post_max - 5MB capa - overhead)
  if (totalAudioSize > maxTotalSize) {
    toastr.error(
      `O tamanho total das faixas é de ${(
        totalAudioSize /
        (1024 * 1024)
      ).toFixed(
        1
      )} MB, mas o máximo permitido é 24 MB. Reduz o bitrate ou o número de faixas.`
    );
    btn.innerHTML = '<i class="bi bi-send-fill me-2"></i>Distribuir';
    btn.disabled = false;
    return;
  ...
  const stores = [];
  document
    .querySelectorAll(".store-card.selected")
    .forEach((c) => stores.push(c.dataset.storeId));
  ...
  fd.append(
    "label_name",
    CAN_LABEL
      ? document.getElementById("label_name").value.trim()
      : "102022 WU Records"
  );
  ...
  trackCards.forEach((card, index) => {
    const audioFile = card.querySelector(".track-audio").files[0];
    if (audioFile) {
      const safeFileName = `track_${index + 1}_${audioFile.name.replace(
        /[^a-zA-Z0-9.]/g,
        "_"
      )}`;
  ...
  try {
    ...
    if (data.ok) {
      await Swal.fire({
        ...
        iconColor: "#FF0089",
        title:
          '<i class="bi bi-music-note-beamed me-2"></i>Lançamento enviado!',
        html: `<p class="mb-2">O teu lançamento <strong>${
          document.getElementById("title").value
        }</strong> foi submetido com sucesso!</p>
    ...
    Swal.fire({
      ...
      text:
        "Verifica a tua internet e tenta novamente. Detalhes: " + err.message,
      confirmButtonColor: "#FF0089",
    });
  }
}
  ...
  if (e.target.classList.contains("track-audio")) {
    ...
    if (e.target.files[0]) {
      ...
      if (
        ...
        showAudioError(card, "Formato inválido. Usa MP3.");
      } else {
        hideAudioError(card);
      }
    } else {
...
function saveCompleteDraft() {
  const draft = {
    ...
    stores: Array.from(document.querySelectorAll(".store-card.selected")).map(
      (c) => c.dataset.storeId
    ),
  };
