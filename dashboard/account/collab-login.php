...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action_change_password'])) {
    ...
    if (!validateCsrf($_POST['csrf_token'] ?? '')) {
        echo json_encode(['ok'=>false,'message'=>'Sessão expirada. Recarrega a página.']); exit;
    }
    if (empty($_SESSION['collab_id'])) {
        echo json_encode(['ok'=>false,'message'=>'Sessão não encontrada. Faz login novamente.']); exit;
    }
    ...
    if (strlen($new_pwd) < 8) {
        echo json_encode(['ok'=>false,'message'=>'A senha deve ter pelo menos 8 caracteres.']); exit;
    }
    if ($new_pwd !== $conf_pwd) {
        echo json_encode(['ok'=>false,'message'=>'As senhas não coincidem.']); exit;
    }
...
<body>
    ...
    <div class="login-card">
        ...
        <div class="login-header">
            <div style="font-size:2.6rem;line-height:1;margin-bottom:6px"><img
                    src="../../assets/img/brand/wasomupfy_authentic.png" class="img-fluid" width="90" height="90"
                    alt="Wasom Upfy">
            </div>
    ...
    const SELF_URL = '<?php echo rtrim(APP_URL,"/") . "/dashboard/account/collab-login"; ?>';
    const DASH_URL = '<?php echo rtrim(APP_URL,"/") . "/dashboard/collab/overview"; ?>';
