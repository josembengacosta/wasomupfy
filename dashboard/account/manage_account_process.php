...
case 'update_account_profile':
    ...
    $db->prepare("
        ...
        WHERE id_users = ?
    ")->execute([
        $name_artist_band ?: null, $about_user ?: null, $url_user ?: null,
        $tel_user ?: null, $country_user ?: null, $city_user ?: null,
        $id_users
    ]);
...
case 'add_collaborator':
    ...
    $db->prepare("
        ...
        VALUES (?,?,?,?,?,?,?,?,?,'pending',?,0,?,1,?)
    ")->execute([
        $id_users, $first_name, $second_name ?: null, $user_collab,
        $email, $tel ?: null, $photo_url ?: null, $role,
        $hash_pwd, $invite_token, $invite_expires, $notes ?: null
    ]);
...
case 'edit_collaborator':
    ...
    ")->execute([
        $first_name, $second_name ?: null, $tel ?: null,
        $photo_url ?: $collab['photo_collab'], $role, $hash_pwd, $notes ?: null,
        $id_collab, $id_users
    ]);
    ...
    if ($resend_invite) {
        ...
        $body = "<div style='font-family:Arial,sans-serif;max-width:540px;margin:auto'>
            ...
            <div style='background:#fff;padding:28px 32px;border:1px solid #eee;border-top:none;border-radius:0 0 8px 8px'>
                ...
                <p><a href='".rtrim(APP_URL,'/')."/dashboard/account/collab-login' style='background:#FF0089;color:#fff;padding:10px 24px;border-radius:8px;text-decoration:none'>Entrar no painel</a></p>
                <p style='color:#999;font-size:.8rem'>".APP_NAME." &mdash; Não respondas.</p>
            </div>
        </div>";
        try { sendEmail($collab['email_collab'], 'Senha actualizada — '.APP_NAME, $body); } catch(Exception $e) {}
...
case 'resend_invite':
    ...
    try {
        sendCollabInviteEmail($collab['email_collab'], $collab['first_name'], $collab['user_collab'],
            $new_pwd, $role_labels[$collab['role_collab']] ?? 'Editor', $owner_name, $plan_name, $activate_url);
    } catch (Exception $e) { error_log($e->getMessage()); }
