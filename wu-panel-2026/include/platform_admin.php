...
// Arquivo: admin/include/platform_admin.php
// ══════════════════════════════════════════════
...
// ── Hash da senha do admin (para validação em ações sensíveis) ──
$admin_password_hash = '';
if ($admin_id) {
    try {
        $pwd_stmt = $db->prepare("SELECT password_employees FROM _employees WHERE id_employees = ?");
        $pwd_stmt->execute([$admin_id]);
        $admin_password_hash = $pwd_stmt->fetchColumn() ?: '';
    } catch (Exception $e) {
        error_log('[ADMIN PWD] ' . $e->getMessage());
    }
}
