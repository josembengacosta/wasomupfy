...
if ($r_count >= 5) {
    ...
    adminRedirect('/' . ADMIN_PATH . '/forgot-password', ['msg' => 'limit']);
}
...
if (!empty($_POST['hp_field'])) {
    sleep(2);
    adminRedirect('/' . ADMIN_PATH . '/forgot-password', ['msg' => 'sent']);
}
...
if (in_array($admin['status_employees'], $inactive)) {
    ...
    logAudit(
        null,
        null,
        'auth.reset_inactive_account',
        'employees',
        $id,
        ['status' => $admin['status_employees']],
        null
    );
    adminRedirect('/' . ADMIN_PATH . '/forgot-password', ['msg' => 'sent']);
}
