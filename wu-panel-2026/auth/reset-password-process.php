...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    adminRedirect('/' . ADMIN_PATH . '/forgot-password');
}

// CSRF
$csrf = $_POST['csrf_token'] ?? '';
if (!validateAdminCsrf($csrf)) {
    adminRedirect('/' . ADMIN_PATH . '/forgot-password', ['msg' => 'error']);
}
...
if (!$valid_id || $valid_id !== $admin_id) {
    ...
    adminRedirect('/' . ADMIN_PATH . '/forgot-password', ['msg' => 'expired']);
}
...
if (empty($new_password) || empty($confirm_password)) {
    adminRedirect('/' . ADMIN_PATH . '/reset-password', ['token' => $token, 'msg' => 'error']);
}

// Senhas coincidem
if (!hash_equals($new_password, $confirm_password)) {
    adminRedirect('/' . ADMIN_PATH . '/reset-password', ['token' => $token, 'msg' => 'mismatch']);
}
...
if ($weak || strlen($new_password) > 128) {
    adminRedirect('/' . ADMIN_PATH . '/reset-password', ['token' => $token, 'msg' => 'weak']);
}
...
adminRedirect('/' . ADMIN_PATH . '/login', ['msg' => 'reset_ok']);
