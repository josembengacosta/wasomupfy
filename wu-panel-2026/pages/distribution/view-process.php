...
// Arquivo: wu-panel-2026/pages/distribution/view-process.php
// Rota:    wu-panel-2026/releases/view-process (POST only)
// ══════════════════════════════════════════════════════════════
...
if ($action === 'edit_album') {
    ...
    try {
        ...
        $db->prepare("
            ...
            WHERE id_album = ?
        ")->execute([
            $title_album, $genre_main,
            $type_album, $new_status,
            $release_date, $territory, $label_name,
            $id_album
        ]);
        ...
        if ($status_changed) {
            ...
            notifyUser(
                $db, $album['id_users'], $admin_id, 'info',
...
if ($action === 'delete_album') {
    ...
    try {
        ...
        notifyUser(
            $db, $album['id_users'], $admin_id, 'warning',
...
if ($action === 'undelete_album') {
    ...
    try {
        ...
        notifyUser(
            $db, $album['id_users'], $admin_id, 'success',
            'Álbum recuperado ✅',
