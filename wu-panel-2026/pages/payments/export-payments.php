<?php
// ═══════════════════════════════════════════════════════════════════════════════
// WASOM UPFY v2.0 — Exportar Pagamentos (CSV/PDF)
// Arquivo: wu-panel-2026/pages/payments/export-payments.php
// Rota:    wu-panel-2026/payments/export?format=csv&filtros...
// ═══════════════════════════════════════════════════════════════════════════════
...
// Replicar os mesmos filtros de all-payments.php (para manter consistência)
$f_id       = trim($_GET['id'] ?? '');
...
if ($format === 'csv') {
    ...
    fputcsv($output, ['ID', 'Referência', 'Utilizador', 'E-mail', 'Plano', 'Valor (AOA)', 'Método', 'Estado', 'Data']);
    foreach ($rows as $row) {
        fputcsv($output, [
            $row['id_payment'],
            $row['payment_ref'],
    ...
    fclose($output);
    exit;
}// Para PDF, geramos uma página HTML formatada para impressão
header('Content-Type: text/html; charset=utf-8');

// Replicar filtros (mesmo SQL de all-payments.php)
$f_id       = trim($_GET['id'] ?? '');
$f_user     = trim($_GET['user'] ?? '');
$f_plan     = trim($_GET['plan'] ?? '');
$f_status   = trim($_GET['status'] ?? '');
