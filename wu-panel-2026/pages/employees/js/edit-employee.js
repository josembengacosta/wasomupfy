...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let employeeData = {
    ...
    plan: "Enterprise",
    notes:
      "Funcionária responsável pelo departamento financeiro. Acesso total ao sistema.",
  ...
  function loadEmployeeData() {
    ...
    document.getElementById("employeeLocation").textContent = `${
      employeeData.city
    }, ${translateCountry(employeeData.country)}`;
    document.getElementById("employeeSince").textContent = formatDate(
      employeeData.createdAt
    );
    document.getElementById("employeeStatus").textContent = translateStatus(
      employeeData.status
    );
    document.getElementById(
      "employeeStatus"
    ).className = `status-badge status-${employeeData.status}`;
    ...
    rolesContainer.innerHTML = employeeData.roles
      .map(
        (role) =>
          `<span class="badge bg-primary badge-role me-2">${translateRole(
            role
          )}</span>`
      )
      .join("");
  ...
  function loadPermissions() {
    // Análises
    document.getElementById("permissionAnalyticsView").checked =
      permissions.analytics.view;
    document.getElementById("permissionReportsGenerate").checked =
      permissions.analytics.reports;
    document.getElementById("permissionExportData").checked =
      permissions.analytics.export;
    document.getElementById("permissionCustomReports").checked =
      permissions.analytics.custom;

    // Músicas
    document.getElementById("permissionMusicReview").checked =
      permissions.music.review;
    document.getElementById("permissionMusicApprove").checked =
      permissions.music.approve;
    document.getElementById("permissionMusicReject").checked =
      permissions.music.reject;
    document.getElementById("permissionMusicEdit").checked =
      permissions.music.edit;

    // Usuários
    document.getElementById("permissionUsersView").checked =
      permissions.users.view;
    document.getElementById("permissionUsersCreate").checked =
      permissions.users.create;
    document.getElementById("permissionUsersEdit").checked =
      permissions.users.edit;
    document.getElementById("permissionUsersDelete").checked =
      permissions.users.delete;

    // Distribuição
    document.getElementById("permissionDistributionView").checked =
      permissions.distribution.view;
    document.getElementById("permissionReleasesManage").checked =
      permissions.distribution.releases;
    document.getElementById("permissionScheduleRelease").checked =
      permissions.distribution.schedule;
    document.getElementById("permissionStoreManage").checked =
      permissions.distribution.stores;

    // Financeiro
    document.getElementById("permissionPaymentsView").checked =
      permissions.financial.payments;
    document.getElementById("permissionEarningsView").checked =
      permissions.financial.earnings;
    document.getElementById("permissionProcessPayments").checked =
      permissions.financial.process;
    document.getElementById("permissionFinancialReports").checked =
  ...
  function save2FASettings() {
    const twoFAMethod = document.querySelector(
      'input[name="2faMethod"]:checked'
    ).id;
        ...
        simulateAPICall("update2FA", {
          ...
          .then((response) => {
            if (response.success) {
              ...
              // Registrar atividade
              logActivity(
                enable2FA.checked ? "2FA ativado" : "2FA desativado",
                "security"
              );
            } else {
  ...
  function resetForm() {
    loadEmployeeData();
    showNotification(
      "Formulário redefinido para os valores originais.",
      "info"
    );
  }
  ...
  function resetPermissions() {
    loadPermissions();
    showNotification(
      "Permissões redefinidas para os valores originais.",
      "info"
    );
  }
