...
const sampleVisitors = [
  ...
  },
  {
    ...
    visitCount: 3,
    pagesVisited: ["/home", "/about.html", "/contact"],
    sessionDuration: 650,
    isBlocked: true,
    isSuspicious: false,
    isBot: false,
...
$(document).ready(function () {
            ...
            return `
                            <div>
                                <div>
                                    ...
                                    <span class="badge bg-light text-dark geo-badge">${
                                      data.city
                                    }</span>
                                </div>
                                <small class="text-muted connection-info">
                                    ...
                                    ${
                                      data.vpn
                                        ? '<span class="badge bg-warning ms-1">VPN</span>'
                                        : ""
                                    }
                                    ${
                                      data.proxy
                                        ? '<span class="badge bg-info ms-1">Proxy</span>'
                                        : ""
                                    }
                                </small>
                            </div>
        ...
        },
        {
          ...
          render: function (data) {
            return `
                            <div>
                                <div>
                                    <i class="bi ${getOSIcon(
                                      data.os
                                    )} os-icon"></i>
                                    <span class="badge bg-secondary device-badge">${
                                      data.os
                                    } ${data.osVersion}</span>
                                    <i class="bi ${getBrowserIcon(
                                      data.browser
                                    )} browser-icon"></i>
                                    <span class="badge bg-primary device-badge">${
                                      data.browser
                                    } ${data.browserVersion}</span>
                                </div>
                                <div class="mt-1">
  ...
  function updateStats() {
    const onlineCount = sampleVisitors.filter((v) =>
      isVisitorOnline(v.lastVisit)
    ).length;
    const totalCount = sampleVisitors.length;
    const blockedCount = sampleVisitors.filter((v) => v.isBlocked).length;
    const countries = [...new Set(sampleVisitors.map((v) => v.countryName))]
      .length;
  ...
  function loadCountryOptions() {
    const countries = [
      ...new Set(sampleVisitors.map((v) => v.countryName)),
    ].sort();
  ...
  function loadActivityTimeline() {
    ...
    allActivities.sort(
      (a, b) =>
        new Date(`2000-01-01T${b.time}`) - new Date(`2000-01-01T${a.time}`)
    );
  ...
  $(document).on("click", ".view-visitor-btn", function () {
    ...
    if (currentVisitor) {
      ...
      if (currentVisitor.vpn)
        securityBadges.push('<span class="badge bg-warning">VPN</span>');
      if (currentVisitor.proxy)
        securityBadges.push('<span class="badge bg-info">Proxy</span>');
      if (currentVisitor.isBot)
        securityBadges.push(
          '<span class="badge bg-primary">Bot/Crawler</span>'
        );
      ...
      $("#view-visitor-body").html(`
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-body text-center">
                                ...
                                <h5>${
                                  currentVisitor.userId
                                    ? `Usuário #${currentVisitor.userId}`
                                    : "Visitante Anônimo"
                                }</h5>
                                <p class="text-muted">
                                ...
                                <div class="mt-3">
                                    ...
                                    <table class="table table-sm">
                                        ...
                                        <tr>
                                            ...
                                            <td>${
                                              currentVisitor.visitCount
                                            }</td>
                                        </tr>
                                        <tr>
                                            <th style='color:#718096!important'>Duração Sessão:</th>
                                            <td>${formatDuration(
                                              currentVisitor.sessionDuration
                                            )}</td>
                                        </tr>
                                    </table>
                                </div>
        ...
        currentVisitor.browserVersion
                                            ...
                                            <tr>
                                                ...
                                                <td>${
                                                  currentVisitor.screenResolution
                                                }</td>
                                            </tr>
                                            <tr>
                                                <th style='color:#718096!important'>Idioma:</th>
                                                <td>${
                                                  currentVisitor.language
                                                }</td>
                                            </tr>
                                            <tr>
                                                <th style='color:#718096!important'>Conexão:</th>
                                                <td>${
                                                  currentVisitor.connectionType
                                                }</td>
                                            </tr>
                                        </table>
                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <table class="table table-sm">
                                    <tr>
                                        ...
                                        <td>${
                                          currentVisitor.referrer || "Direto"
                                        }</td>
                                    </tr>
                                    <tr>
                                        <th style='color:#718096!important'>User Agent:</th>
                                        <td><small class="text-muted">${
                                          currentVisitor.userAgent
                                        }</small></td>
                                    </tr>
                                </table>
                            </div>
  ...
  $(document).on("click", ".block-visitor-btn", function () {
    ...
    if (currentVisitor) {
      ...
      $("#block-visitor-details").html(`
                <div class="alert ${
                  isAlreadyBlocked ? "alert-info" : "alert-warning"
                }">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-sm">
                            ...
                            <tr>
                                ...
                                <td>
                                    <i class="flag-icon flag-icon-${
                                      countryFlags[currentVisitor.country] ||
                                      "globe"
                                    }"></i>
                                    ${currentVisitor.countryName}, ${
        currentVisitor.city
      ...
      const title = isAlreadyBlocked
        ? "Desbloquear Visitante"
        : "Bloquear Visitante";
      $("#blockVisitorModal .modal-title").html(
  ...
  $("#block-visitor-form").submit(function (e) {
    ...
    if (currentVisitor) {
      ...
      showToast(
        ...
        currentVisitor.isBlocked
          ? "Visitante Bloqueado"
          : "Visitante Desbloqueado",
        `O visitante ${currentVisitor.ip} foi ${
          currentVisitor.isBlocked ? "bloqueado" : "desbloqueado"
        } com sucesso.`
      );
    }
  });
  ...
  $(document).on("click", ".delete-visitor-btn", function () {
    ...
    if (
      visitor &&
      confirm(`Tem certeza que deseja excluir o visitante ${visitor.ip}?`)
    ) {
      ...
      if (index !== -1) {
        ...
        showToast(
          "success",
          "Visitante Excluído",
          `O visitante ${visitor.ip} foi removido.`
        );
      }
    }
  });
  ...
  $("#export-selected-btn").click(function () {
    if (selectedVisitors.size === 0) {
      showToast(
        ...
        "Selecione pelo menos um visitante para exportar."
      );
      return;
    }
  ...
  $("#block-selected-btn").click(function () {
    if (selectedVisitors.size === 0) {
      showToast(
        "warning",
        "Nenhum item selecionado",
        "Selecione pelo menos um visitante para bloquear."
      );
      return;
    }
    ...
    if (
      confirm(
        `Deseja bloquear ${selectedVisitors.size} visitante(s) selecionado(s)?`
      )
    ) {
      ...
      showToast(
        ...
        `${selectedVisitors.size} visitante(s) foram bloqueados.`
      );
    }
  });
  ...
  $("#delete-selected-btn").click(function () {
    if (selectedVisitors.size === 0) {
      showToast(
        "warning",
        "Nenhum item selecionado",
        "Selecione pelo menos um visitante para excluir."
      );
      return;
    }
    ...
    if (
      confirm(
        `Deseja excluir permanentemente ${selectedVisitors.size} visitante(s) selecionado(s)?`
      )
    ) {
      // Remover visitantes selecionados
      sampleVisitors = sampleVisitors.filter(
        (v) => !selectedVisitors.has(v.id)
      );
      initDataTable();
      selectedVisitors.clear();
      updateSelectedCount();
      showToast(
        "success",
        "Visitantes Excluídos",
        `${selectedVisitors.size} visitante(s) foram removidos.`
      );
    }
  });
  ...
  $("#export-form").submit(function (e) {
    ...
    if (dataToExport.length === 0) {
      showToast(
        "warning",
        "Sem dados",
        "Não há dados para exportar com os critérios selecionados."
      );
      return;
    }
    ...
    switch (format) {
      ...
      case "pdf":
        ...
        showToast(
          ...
          "A exportação para PDF está em desenvolvimento."
        );
        break;
      ...
      case "excel":
        showToast(
          ...
          "A exportação para Excel está em desenvolvimento."
        );
        break;
    }
  ...
  $("#bulk-delete-form").submit(function (e) {
    ...
    if (confirmText !== "EXCLUIR TUDO") {
      showToast(
        ...
        'Digite exatamente "EXCLUIR TUDO" para confirmar.'
      );
      return;
    }
    ...
    showToast(
      "success",
      "Limpeza Concluída",
      `${deleteCount} registro(s) foram removidos.`
    );
  });
  ...
  $("#delete-visitor-btn").click(function () {
    if (
      currentVisitor &&
      confirm(`Excluir permanentemente o visitante ${currentVisitor.ip}?`)
    ) {
