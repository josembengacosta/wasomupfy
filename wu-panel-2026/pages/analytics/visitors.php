...
// Arquivo: wu-panel-2026/pages/analytics/visites.php
// Rota:    wu-panel-2026/analytics/visitors
// ══════════════════════════════════════════════
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($action === 'delete' && $visitor_id) {
        ...
        if (!$adm_row || !password_verify($pw, $adm_row['password_employees'])) {
            echo json_encode(['ok' => false, 'message' => 'Senha incorrecta.']); exit;
        }
        try {
    ...
    if ($action === 'bulk_delete') {
        ...
        try {
            ...
            if (!$sql) { echo json_encode(['ok' => false, 'message' => 'Opção inválida.']); exit; }
            $count = $db->exec($sql);
...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    if (!empty($_GET['status']))  { $where[] = 'v.status_visitor = ?';      $params[] = $_GET['status']; }
    if (!empty($_GET['device']))  { $where[] = 'v.device_type = ?';         $params[] = $_GET['device']; }
    if (!empty($_GET['browser'])) { $where[] = 'v.browser = ?';             $params[] = $_GET['browser']; }
    if (isset($_GET['bot']) && $_GET['bot'] !== '') { $where[] = 'v.is_bot = ?'; $params[] = (int)$_GET['bot']; }
    if (!empty($_GET['date_from'])){ $where[] = 'DATE(v.creat_visitor) >= ?'; $params[] = $_GET['date_from']; }
    if (!empty($_GET['date_to']))  { $where[] = 'DATE(v.creat_visitor) <= ?'; $params[] = $_GET['date_to']; }
...
$where = []; $params = [];
if ($f_ip)      { $where[] = 'v.ip_address LIKE ?';     $params[] = '%'.$f_ip.'%'; }
if ($f_country) { $where[] = 'v.country_code = ?';       $params[] = strtoupper($f_country); }
if ($f_status)  { $where[] = 'v.status_visitor = ?';     $params[] = $f_status; }
if ($f_device)  { $where[] = 'v.device_type = ?';        $params[] = $f_device; }
if ($f_browser) { $where[] = 'v.browser = ?';            $params[] = $f_browser; }
if ($f_bot !== '') { $where[] = 'v.is_bot = ?';          $params[] = (int)$f_bot; }
if ($f_from)    { $where[] = 'DATE(v.creat_visitor) >= ?'; $params[] = $f_from; }
if ($f_to)      { $where[] = 'DATE(v.creat_visitor) <= ?'; $params[] = $f_to; }
$sql_where = $where ? 'WHERE '.implode(' AND ', $where) : '';
...
function vis_device_icon(string $d): string {
    return match(strtolower($d)) {
        'desktop' => 'bi-pc-display', 'mobile' => 'bi-phone', 'tablet' => 'bi-tablet',
        'bot'     => 'bi-robot',      default  => 'bi-question-circle',
    };
}
function vis_browser_icon(string $b): string {
    return match(strtolower($b)) {
        'chrome' => 'bi-browser-chrome', 'firefox' => 'bi-browser-firefox',
        'safari' => 'bi-browser-safari', 'edge' => 'bi-browser-edge',
        default  => 'bi-globe2',
    };
}
                ...
                <div class="card p-0" style="border-radius:14px;overflow:hidden">
                    ...
                    <div class="d-flex justify-content-center py-3">
                        <nav>
                            ...
                            if($s>1){echo '<li class="page-item"><a class="page-link" href="?'.http_build_query(array_merge($_GET,['page'=>1])).'">1</a></li>';
