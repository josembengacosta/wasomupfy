...
// Arquivo: wu-panel-2026/pages/analytics/online-users.php
// Rota:    wu-panel-2026/analytics/online-users
// ══════════════════════════════════════════════
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($action === 'broadcast') {
        ...
        if (empty($message)) {
            echo json_encode(['ok' => false, 'message' => 'A mensagem é obrigatória.']); exit;
        }
        ...
        if ($audience === 'role') {
            ...
            $where_b = 'u.status_user = ?'; $params_b[] = 'active';
        } elseif ($audience === 'country') {
            $where_b = 'u.country_user = ?'; $params_b[] = $aud_val;
        } elseif ($audience === 'selected') {
            $ids = array_map('intval', explode(',', $_POST['selected_ids'] ?? ''));
            $ids = array_filter($ids);
            if (empty($ids)) { echo json_encode(['ok' => false, 'message' => 'Nenhum utilizador seleccionado.']); exit; }
            $ph = implode(',', array_fill(0, count($ids), '?'));
            $where_b = "u.id_users IN ($ph)"; $params_b = $ids;
        }
        ...
        if ($count === 0) {
            echo json_encode(['ok' => false, 'message' => 'Nenhum destinatário encontrado.']); exit;
        }
    ...
    if ($action === 'send_message') {
        ...
        if (!$to_user || empty($body)) {
            echo json_encode(['ok' => false, 'message' => 'Destinatário e mensagem são obrigatórios.']); exit;
        }
        try {
            $db->prepare("
            ...
            $db->prepare("
                ...
                VALUES (?, ?, 'info', 'Nova mensagem do Admin', ?, '/dashboard/page/notifications')
    ...
    if ($action === 'force_logout') {
        ...
        if (!$uid) { echo json_encode(['ok'=>false,'message'=>'ID inválido.']); exit; }
        try {
            $db->prepare("UPDATE _users_sessions SET is_active=0 WHERE id_users=?")->execute([$uid]);
            $db->prepare("UPDATE _user_presence SET online_status='offline' WHERE id_users=?")->execute([$uid]);
    ...
    if ($action === 'refresh') {
        ...
        $where = ["up.online_status != 'offline'", "up.last_activity >= DATE_SUB(NOW(), INTERVAL 30 MINUTE)"];
        $params = [];
        if ($f_status && in_array($f_status, ['online','away','busy','invisible'])) {
            $where[] = 'up.online_status = ?'; $params[] = $f_status;
        }
        if ($f_plan) {
            $where[] = 'u.plan_selected = ?'; $params[] = $f_plan;
        }
        ...
        $counts = $db->query("
            ...
            FROM _user_presence
            WHERE online_status != 'offline' AND last_activity >= DATE_SUB(NOW(),INTERVAL 30 MINUTE)
    ...
    echo json_encode(['ok' => false, 'message' => 'Acção desconhecida.']); exit;
}
...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    $stmt = $db->query("
        ...
        JOIN _users u ON u.id_users = up.id_users
        LEFT JOIN _plans p ON p.id_plan = u.plan_selected
        WHERE up.online_status != 'offline' AND up.last_activity >= DATE_SUB(NOW(),INTERVAL 30 MINUTE)
        ORDER BY up.last_activity DESC
    ");
    $rows = $stmt->fetchAll();
    ...
    fputcsv($out, ['ID','Nome','E-mail','Estado','Última Actividade','Tipo Actividade','Última Página','Dispositivo','Browser','IP','País','Cidade','Duração Sessão (s)','Início Sessão','Plano']);
    foreach ($rows as $r) {
        fputcsv($out, [
            $r['id_users'], trim($r['first_name'].' '.($r['second_name']??'')),
            $r['email_user'], $r['online_status'], $r['last_activity'],
            $r['last_activity_type'], $r['last_page'], $r['device_type'],
            $r['browser'], $r['ip_address'], $r['country_name'], $r['city'],
            $r['session_duration'], $r['session_start'], $r['name_plan'],
        ]);
    }
    fclose($out); exit;
}
...
function ou_device_icon(string $d): string {
    return match(strtolower($d)) {
        'desktop' => 'bi-pc-display', 'mobile' => 'bi-phone',
        'tablet'  => 'bi-tablet',     default  => 'bi-display',
    };
}
...
function ou_color(string $name): string {
    $colors = ['#FF0089','#f97316','#8b5cf6','#06b6d4','#22c55e','#eab308','#ec4899','#3b82f6','#14b8a6'];
    return $colors[abs(crc32($name)) % count($colors)];
}
function ou_activity_label(?string $t): string {
    if (!$t) return '—';
    return match($t) {
        'listening'  => '🎵 Ouvindo', 'uploading' => '📤 Uploading',
        'dashboard'  => '📊 Dashboard', 'releases' => '🎵 Lançamentos',
        'finances'   => '💰 Finanças', 'profile' => '👤 Perfil',
        'artists'    => '🎤 Artistas', 'settings' => '⚙️ Definições',
                ...
                <div class="row mb-3 mt-2 align-items-center">
                    ...
                    <div class="col-auto ms-auto d-flex align-items-center gap-2 flex-wrap">
                        <span class="refresh-badge" id="refresh-badge">
                            ...
                            Actualiza em <span id="countdown">30</span>s
                        </span>
                ...
                foreach ($ou_cards as [$val,$lbl,$color,$icon]): ?>
                    <div class="col-6 col-md-<?php echo count($ou_cards)<=4?'3':'2'; ?>">
                        <div class="ou-stat">
                            ...
                            <div>
                                <div class="ou-stat-num" id="stat-<?php echo strtolower($lbl); ?>"><?php echo $val; ?>
                                </div>
                ...
                <div class="row g-3">
                                    ...
                                    <div class="d-flex align-items-start gap-3">
                                        ...
                                        <div style="position:relative;flex-shrink:0">
                                            ...
                                            <div class="ou-avatar-ini"
                                                style="background:<?php echo $clr; ?>;display:none"><?php echo $ini; ?>
                                            </div>
                                            <?php else: ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <div style="font-size:.7rem;opacity:.4">
                                                        <?php echo htmlspecialchars(ucfirst($u['browser'])); ?></div>
                                                    <?php endif; ?>
                                                </td>
                                                ...
                                                <td style="font-size:.78rem">
                                                    ...
                                                    <div style="font-size:.7rem;opacity:.4">
                                                        <?php echo date('H:i',strtotime($u['session_start'])); ?></div>
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <?php if (!empty($u['name_plan'])): ?>
                    ...
                    <div class="col-lg-3">
                        ...
                        <div class="card p-3 mb-3" style="border-radius:14px">
                            ...
                            <div style="opacity:.35;font-size:.78rem;text-align:center;padding:12px 0">Sem dados</div>
                            <?php else: ?>
                        ...
                        <div class="card p-3" style="border-radius:14px;text-align:center">
                            ...
                            <div style="font-size:.9rem;font-weight:700" id="last-update"><?php echo date('H:i:s'); ?>
                            </div>
                            <div style="font-size:.72rem;opacity:.4;margin-top:4px"><?php echo date('d/m/Y'); ?></div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div style="font-size:.72rem;opacity:.45;margin-top:3px">A mensagem será inserida em
                            <em>_notification</em> e exibida no dashboard de cada utilizador.</div>
                    </div>
                </div>
    ...
    (function() {
        ...
        document.getElementById('btn-send-broadcast')?.addEventListener('click', async function() {
            ...
            try {
                ...
                if (d.ok) bootstrap.Modal.getInstance(document.getElementById('broadcastModal')).hide();
            } catch {
        ...
        document.getElementById('btn-send-message')?.addEventListener('click', async function() {
            ...
            try {
                ...
                toast(d.ok ? 'success' : 'danger', d.message);
                if (d.ok) bootstrap.Modal.getInstance(document.getElementById('messageModal')).hide();
            } catch {
                toast('danger', 'Erro de ligação.');
            }
            this.disabled = false;
        ...
        window.forceLogout = async function(uid, name) {
            if (!confirm(`Forçar logout de "${name}"? O utilizador será desligado imediatamente.`)) return;
            try {
                const d = await postAction({
