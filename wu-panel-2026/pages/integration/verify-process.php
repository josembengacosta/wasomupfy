...
// Arquivo: wu-panel-2026/pages/integration/verify-process.php
// ══════════════════════════════════════════════════════════════
...
function sendChannelEmail(string $to, string $name, string $channel_name, string $status, string $reason = ''): void
{
    ...
    if (file_exists($mailer_path)) {
        ...
        try {
            ...
            $wm->send();
        } catch (Exception $e) { error_log('[CHANNEL_EMAIL] ' . $e->getMessage()); }
    }
}
...
if ($action === 'verify') {
    ...
    } catch (Exception $e) { jOut(false, 'Erro ao verificar.'); }
}
...
if ($action === 'reject') {
    ...
    } catch (Exception $e) { jOut(false, 'Erro ao rejeitar.'); }
}
...
if ($action === 'remove') {
    ...
    } catch (Exception $e) { jOut(false, 'Erro ao remover.'); }
}
